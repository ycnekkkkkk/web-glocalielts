...
import {
  ArrowLeft, BookOpen, Calendar, CheckCircle, Download, MessageSquare, Star, Users, WrapText,
} from "lucide-react";
import Link from "next/link";
import { use, useEffect, useState } from "react";
import type { ClassEvaluationRow, MakeupStatusRow } from "@/types";

type TabId = "overview" | "attendance" | "evaluations" | "makeup";
...
export default function AcademicManagerClassDetailPage({ params }: { params: Promise<{ id: string }> }) {
  ...
  useEffect(() => {
    async function load() {
      ...
      // Derive taught sessions from real attendance rows for this class.
      ...
      for (const row of (attendanceData || []) as AttendanceRow[]) {
        ...
        seen.add(`${row.class_name}__${row.session_ref}`);
      }
      const taughtFromAttendance = seen.size;
      ...
      setClassDetail({
        ...
        sessions_done: Math.max(c.sessions_done ?? 0, taughtFromAttendance),
  ...
  const progress = classDetail.total_sessions > 0
    ? Math.round((classDetail.sessions_done / classDetail.total_sessions) * 100)
    : 0;
    ...
    <PageWrapper>
      ...
      <div className="grid grid-cols-2 sm:grid-cols-4 gap-4 mb-6">
        ...
        <Card className="p-4 flex items-center gap-3">
          ...
          <div>
            <p className="text-xl font-bold text-gray-900">{classDetail.sessions_done}/{classDetail.total_sessions}</p>
            ...
            <div className="flex flex-col items-center justify-center py-6 gap-4">
              ...
              <div className="text-center">
                <p className="text-gray-600 text-sm">
                  <span className="font-semibold text-gray-900">{classDetail.sessions_done}</span>
                  <span className="text-gray-400"> / </span>
                  <span className="font-semibold text-gray-900">{classDetail.total_sessions}</span>
