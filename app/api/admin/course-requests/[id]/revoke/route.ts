...
// POST /api/admin/course-requests/[id]/revoke
export async function POST(req: NextRequest, { params }: { params: Promise<{ id: string }> }) {
  try {
    ...
    await supabase.from("notifications").insert({
      user_id: row.user_id,
      title: "Quyền truy cập khóa học bị thu hồi",
      content: `Quyền truy cập khóa học "${row.course_title}" của bạn đã bị thu hồi.${admin_note ? ` Lý do: ${admin_note}` : " Vui lòng liên hệ Admin để biết thêm chi tiết."}`,
    ...
    if (admin_note && admin_note.trim()) {
      await supabase.from("course_messages").insert({
        ...
        sender_id: admin.id,
        sender_role: "admin",
        content: `[THU HỒI KHÓA HỌC] ${admin_note.trim()}`,
      });
    }
