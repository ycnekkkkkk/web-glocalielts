...
export default function PublicCourseLearnPage({ params }: { params: Promise<{ slug: string }> }) {
  ...
  useEffect(() => {
    async function load() {
      ...
      const { data } = await supabase
        ...
        .eq("status", "published")
        .maybeSingle();
      const found = (data as PublicCourse | null) ?? null;
      setCourse(found);
  ...
  const selectedLessonAssignments =
    selected && Array.isArray(curriculum[selected.lessonIndex]?.assignments)
      ? curriculum[selected.lessonIndex].assignments
      : [];
                  ...
                  return (
                    <div key={`${lesson.name}-${lessonIdx}`} className="rounded-xl border border-gray-100 p-3">
                      ...
                      {(lesson.assignments ?? []).length > 0 ? (
                        <p className="mt-1 text-xs text-gray-500">Bài tập: {(lesson.assignments ?? []).length}</p>
                      ) : null}
                ...
                <>
                  ...
                  {selectedLessonAssignments.length > 0 ? (
                    <div className="mt-4 rounded-xl border border-gray-200 bg-gray-50/70 p-3">
                      <p className="text-sm font-semibold text-gray-900">Bài tập của lesson này</p>
                      <ul className="mt-2 space-y-1.5 text-sm text-gray-700 list-disc pl-5">
                        {selectedLessonAssignments.map((item, idx) => (
